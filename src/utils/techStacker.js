...
async function analyzeTechStack(workspacePath) {
    ...
    const find = async (pattern) => {
        return new Promise((resolve, reject) => {
            ...
            glob(fullPattern, { ignore: '**/node_modules/**' }, (err, files) => {
                if (err) reject(err);
                else resolve(files.map(f => ({ fsPath: f })));
            });
        });
    };
    ...
    const readFile = async (filePath) => {
        try {
            return await fs.readFile(filePath.fsPath || filePath, 'utf-8');
        } catch (error) {
 ...
 */
function generateTechStackReport(techStackData) {
 ...
 */
function getDomainFocus(primaryStack) {
 ...
 */
function getIntegrationRecommendations(techStackData) {
...
module.exports = { analyzeTechStack, generateTechStackReport };
