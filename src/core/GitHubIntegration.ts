...
export class GitHubIntegration {
   ...
   */
  async initialize(): Promise<GitHubIntegrationResult> {
    try {
      const tasks: Promise<GitHubIntegrationResult>[] = [];
   ...
   */
  async updateGitIgnore(): Promise<GitHubIntegrationResult> {
    try {
      const gitignorePath = path.join(this.basePath, '.gitignore');
      
      const lcagentsEntries: GitIgnoreEntry[] = [
        {
   ...
   */
  async createCopilotWorkflows(): Promise<GitHubIntegrationResult> {
    try {
      ...
      await fs.writeFile(
        path.join(workflowsDir, 'lcagents-validation.yml'),
        validationWorkflow,
        'utf-8'
      );
      ...
      await fs.writeFile(
        path.join(workflowsDir, 'lcagents-docs.yml'),
        docsWorkflow,
        'utf-8'
      );
   ...
   */
  async createGitHubTemplates(): Promise<GitHubIntegrationResult> {
    try {
      ...
      await fs.writeFile(
        path.join(issueTemplatesDir, 'agent-request.md'),
      ...
      await fs.writeFile(
        path.join(issueTemplatesDir, 'bug-report.md'),
        bugReportTemplate,
        'utf-8'
      );
   ...
   */
  private generateValidationWorkflow(): string {
...
`;
   ...
   */
  private generateDocsWorkflow(): string {
...
`;
   ...
   */
  private generateAgentRequestTemplate(): string {
    return `---
name: Agent Request
...
`;
   ...
   */
  getRepositoryInfo(): { organization?: string; repository?: string; branch?: string } {
    const result: { organization?: string; repository?: string; branch?: string } = {
